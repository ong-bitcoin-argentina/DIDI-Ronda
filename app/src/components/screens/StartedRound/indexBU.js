...
import SelectedList from "../roundsCreation/steps/ParticipantSelection/SelectedList";
import DrawModal from "./DrawModal";
import Number from "../../components/Number";
import { roundStatusArray } from "../../utils";

const StartedRound = props => {
  const { navigation } = props;

  const roundParams = {
    name: navigation.getParam("name", "Ronda"),
    paymentsQty: navigation.getParam("paymentsQty", "2"),
    pending: navigation.getParam("pending", false),
    amount: navigation.getParam("amount", "200"),
    frequency: navigation.getParam("frequency", "m"),
    participants: navigation.getParam("participants", []),
    startDate: navigation.getParam("customStartDate", "10/10/10"),
    endDate: navigation.getParam("customEndDate", "10/10/10"),
    _id: navigation.getParam("_id", null),
    admin: navigation.getParam("admin", null),
  };
  ...
  const round = requestRounds.list.find(
    e => e._id === navigation.getParam("_id", null)
  );
  ...
  const lastCompleted = round.shifts
    .reverse()
    .find(e => e.status === "completed");
  round.shifts.reverse();
  const currentShift =
    lastCompleted &&
    round.shifts.find(e => e.number === lastCompleted.number + 1);

  const currentShiftPaysAmount =
    currentShift &&
    currentShift.pays &&
    currentShift.pays.length * round.amount;
  ...
  const currentShiftHeader = shifts.find(
    s => s.status === "current" || s.status === "draw"
  );
  const currentPaymentHeader = currentShiftHeader
    ? currentShiftHeader.number
    : "0";
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollView}>
        ...
        <Header
          ...
          currentShiftPaysAmount={currentShiftPaysAmount || 0}
          name={name}
        />
        ...
        <View style={{ flex: 1 }}>
          <FlatList
            ...
            renderItem={({ item }) => {
              ...
              const numberParticipants = participants.filter(p =>
                item.participant.includes(p._id)
              );
              const numberTitle = numberParticipants
                .map(p => p.user.name)
                .join("/");
              ...
              const numberCurrent =
                currentShift && currentShift.number === item.number;
        ...
        <View style={styles.participantsContainer}>
          <Text style={{ color: "#000", fontWeight: "600", paddingLeft: 20 }}>
            Participantes
          </Text>
          <SelectedList
